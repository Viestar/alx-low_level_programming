...
/**
 * main - Displays ELF elf_head infomation at the start of an ELF file
...
int main(int __attribute__((__unused__)) argc, char *argv[])
{
	...
	/* Using the descriptor too read into elf_head_buffer*/
	...
	elf_checker((*elf_head).e_ident); /* Checking if elf or not*/
	...
	printf(" ABI Version:  %d", (*elf_head).e_ident[EI_ABIVERSION]);

	type_printer((*elf_head).e_type, (*elf_head).e_ident);	 /* Type */
	entry_printer((*elf_head).e_entry, (*elf_head).e_ident); /* Entry point */
...
void file_closer(int file)
{
	...
	if (closer == -1)
	{
		dprintf(STDERR_FILENO, "Error: File %d can't be closed", file);
		exit(98);
	}
}
